...
(defun promotable-to-register-p (type-desc)
  "Return T if a variable of this type can be register-allocated.
   Excludes arrays, structs, unions, long long, and volatile types.
   Pointers (pointer-level > 0) are always promotable regardless of base type:
   a pointer-to-struct is just a 4-byte scalar, not a struct value."
  (and type-desc
       (not (type-desc-array-size type-desc))
       (or (> (type-desc-pointer-level type-desc) 0)
           (not (member (type-desc-base type-desc) '(struct union longlong))))
       (not (type-desc-volatile-p type-desc))))
